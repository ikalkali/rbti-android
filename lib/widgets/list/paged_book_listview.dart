...
class PagedBookListView extends StatefulWidget {
  const PagedBookListView({Key? key, required this.bookRepository})
      : super(key: key);
...
class _PagedBookListViewState extends State<PagedBookListView> {
  ...
  Future<void> _fetchPage(int pageKey) async {
    ...
    try {
      ...
      final newPage = await widget.bookRepository
          .fetchPaginatedBook(pageSize, (pageKey - 1) * pageSize);

      final prevFetchedItems = _pagingController.itemList?.length ?? 0;
      print("prevFetched : $prevFetchedItems");
      final bool isLastPage = prevFetchedItems + pageSize >
          widget.bookRepository.currentPaginateCount;
      if (isLastPage) {
        print("LAST PAGE!");
